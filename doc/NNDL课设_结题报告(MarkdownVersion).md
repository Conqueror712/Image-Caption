...
2. **TransformerEncoder类:**
    - 包含多个Transformer编码器层（`TransformerEncoderLayer`）的堆叠。
    - 提供了前向传播的方法，对输入序列进行多层次的编码处理。
3. **TransformerDecoder类:**
    - 包含多个Transformer解码器层（`TransformerDecoderLayer`）的堆叠。
    - 提供了前向传播的方法，用于处理目标序列，同时依赖来自编码器的信息。
4. **TransformerEncoderLayer类:**
    - 定义了Transformer模型中的编码器层，包含多头自注意力机制和前馈神经网络（feedforward network）。
    - 包含前向传播方法，支持在序列中加入位置编码。
5. **TransformerDecoderLayer类:**
    - 定义了Transformer模型中的解码器层，包含多头自注意力机制、多头注意力机制和前馈神经网络。
    - 支持在序列中加入位置编码。
6. **DecoderEmbeddings类:**
    - 定义了解码器的嵌入层，包括词嵌入和位置嵌入。
    - 在前向传播中，将输入序列进行嵌入处理，并加入位置嵌入。
7. **生成掩码的辅助函数:**
    - `generate_square_subsequent_mask`: 生成用于遮掩序列中未来信息的方形掩码。
8. **辅助函数和参数:**
    - `_get_clones`: 复制给定模块的副本。
    - `_get_activation_fn`: 根据字符串返回相应的激活函数。
    - `_reset_parameters`: 重置模型参数。
...
```
### 4.3 ARCTIC模型
为了与自己实现的模型进行效果上的对比，我复现了老师给的原始模型——ARCTIC的代码，并成功在DeepFashion-MultiModal 数据集上进行训练，得到了相应的评估指标的得分，模型性能具体的对比我将在下一部分进行重点分析。
...
- 在`DeepFashion-MultiModal`数据集上训练两种服饰图像描述模型（老师给的模型（定义为`Original Model`），以及自己写的网格/区域表示、自注意力+注意力的模型结构（定义为`Attention Model`）），并对比这两种模型之间性能的差异：
    - `Original Model`的训练过程及相应的`CIDEr-D`评分：
