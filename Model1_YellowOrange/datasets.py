...
def create_dataset(max_len=64):
    ...
    # 对训练集描述进行编码
    encoded_captions_train = encode_captions(train_captions_data, word_to_idx, max_len)

    # 对测试集描述进行编码
    encoded_captions_test = encode_captions(test_captions_data, word_to_idx, max_len)
...
class ImageTextDataset(Dataset):
    ...
    def __init__(self, image_paths_file, captions_file, transform=None):
        """
        初始化数据集类。
        参数:
